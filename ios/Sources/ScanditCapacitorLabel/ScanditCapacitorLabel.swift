...
public class ScanditCapacitorLabel: CAPPlugin, CAPBridgedPlugin {
    ...
    @objc func updateLabelCaptureSettings(_ call: CAPPluginCall) {
        ...
        labelModule.applyModeSettings(modeId: getModeId(call), modeSettingsJson: settingsJson, result: CapacitorResult(call))
    }
    ...
    @objc func setViewForCapturedLabel(_ call: CAPPluginCall) {
        ...
        guard let jsonData = viewJson.data(using: .utf8),
              ...
              let decodedViewJson = try? JSONDecoder().decode(TappableBase64ImageView.JSON.self, from: viewJsonData) else {
    ...
    @objc func setAnchorForCapturedLabel(_ call: CAPPluginCall) {
        ...
        let anchroForLabel = AnchorForLabel(dataCaptureViewId: dataCaptureViewId, anchorString: anchor, trackingId: trackingId)
    ...
    @objc func setOffsetForCapturedLabel(_ call: CAPPluginCall) {
        ...
        let dataCaptureViewId = getDataCaptureViewId(call)
        let trackingId = call.getInt("trackingId", 0)
        let offsetForLabel = OffsetForLabel(dataCaptureViewId: dataCaptureViewId, offsetJson: offsetJson, trackingId: trackingId)
    ...
    @objc func setViewForCapturedLabelField(_ call: CAPPluginCall) {
        ...
        dispatchMain { [weak self] in
            ...
            let viewForLabel = ViewForLabel(dataCaptureViewId: dataCaptureViewId, view: view, trackingId: trackingId, fieldName: fieldName)
    ...
    @objc func updateLabelCaptureAdvancedOverlay(_ call: CAPPluginCall) {
        ...
        let dataCaptureViewId = getDataCaptureViewId(call)
        labelModule.updateAdvancedOverlay(dataCaptureViewId, overlayJson: advancedOverlayJson, result: CapacitorResult(call))
    }
    ...
    @objc func updateLabelCaptureValidationFlowOverlay(_ call: CAPPluginCall) {
        ...
        let dataCaptureViewId = getDataCaptureViewId(call)
        labelModule.updateValidationFlowOverlay(dataCaptureViewId, overlayJson: overlayJson, result: CapacitorResult(call))
    }
    
    private func getModeId(_ call: CAPPluginCall) -> Int {
        return call.getInt("modeId", 0)
    }
    
    private func getViewId(_ call: CAPPluginCall) -> Int {
        return call.getInt("viewId", 0)
    }
    
    private func getDataCaptureViewId(_ call: CAPPluginCall) -> Int {
        return call.getInt("dataCaptureViewId", 0)
    }
}
